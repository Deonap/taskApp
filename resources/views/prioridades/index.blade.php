<x-app-layout>
    ...
    <nav class="bg-grey-light p-3 rounded font-sans w-full m-4 ">
        <ol class="list-reset flex text-grey-dark ml-8">
            <li>
                <a href="#" class="text-blue-600 hover:text-blue-700 font-bold" style="color: #0a3857; font-size: 1.5em; vertical-align: middle;">Prioridades</a>
            </li>
            <li>
                <span class="mx-2" style="color: #0a3857; font-size: 1.5em; vertical-align: middle;">></span>
            </li>
            <li style="color: #0a3857; font-size: 1.5em; vertical-align: middle;">Definir</li>
        </ol>
    ...
    <div class="container mx-auto px-4">
        <div id="tabelaProjetosAbertos" class="mb-8">
            <div class="flex items-center text-white  mb-4" style="width: 100%;">
                <div class="flex-none" style="width: 70%; height: 40px; background-color: #0a3857; padding: 1rem; border-radius: 0.2rem; display: flex; justify-content: start; align-items: center;">
                ...
                <div class="flex-none ml-4" style="width: 29%; height: 30px; background-color: #f0f1f0; color: black; padding: 1rem; border-radius: 0.2rem; display: flex; justify-content: center; align-items: center;">
                    <p class="text-sm font-medium">Data da Semana: {{ \Carbon\Carbon::now()->startOfWeek()->format('d/m/Y') }} - {{ \Carbon\Carbon::now()->endOfWeek()->format('d/m/Y') }}</p>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                ...
                <thead class="bg-[#d5d4d5]">
                    <tr>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Nº</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Cliente</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Tipo</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Projeto</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;border-right: 2px solid #bbbaba;">Tarefas</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Observações</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Tempo</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Estado</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Ações</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @php
                        $colaboradorProjeto = $projeto->users->where('id', $colaboradorId)->first(); // Encontre o colaborador específico no projeto
                    @endphp
    <tr data-id="{{ $projeto->id }}" data-user-id="{{ optional($projeto->users->first())->id }}">
        ...
        <td class="border px-3 py-4 whitespace-nowrap border-b">
            ...
            @else
                {{ 'Sem prioridade definida' }} {{-- ou qualquer outro valor padrão que desejar --}}
            @endif
        </td>
            ...
            <td class="border px-3 py-4 whitespace-nowrap border-b w-1/4"style="border-right: 2px solid #bbbaba;" > <!-- Ajustando a largura da coluna de tarefas -->
                @foreach($projeto->tarefas as $tarefa)
                    <div>{{ $tarefa->descricao }}</div>
                @endforeach
            </td>
            <td class="border px-3 py-4 whitespace-nowrap border-b w-1/4">
                <textarea class="form-input observacoes border border-gray-300 rounded-md w-full resize-none h-16 overflow-y-auto" rows="3" readonly>
                    {{ $colaboradorProjeto->pivot->observacoes ?? 'Sem observações' }}
                </textarea>
            </td>
            <td class="border px-3 py-4 whitespace-nowrap border-b w-1/12">
                <input type="number" class="border border-gray-300 rounded-md p-2 w-full tempo-gasto" value="{{ $colaboradorProjeto->pivot->tempo_gasto ?? '' }}">
            </td>
            <td class="border px-4 py-2">
                        <div style="background-color: {{ $projeto->estadoProjeto->cor }}; margin: auto;" class="w-7 h-7 rounded-full"></div>
                    </td>
            <td class="border px-3 py-4 whitespace-nowrap border-b">
                <div class="flex justify-center items-center">
                <a href="{{ route('projetos.edit', $projeto->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                ...
                <form action="{{ route('projetos.destroy', $projeto->id) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-red-600 hover:text-red-900"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <div id="tabelaProjetosPendentes" class="mb-8">
        <div class="relative mb-4">
            <div class="flex-none text-white" style="width: 70%; height: 40px; background-color: #f26d23; padding: 1rem; border-radius: 0.2rem; display: flex; justify-content: start; align-items: center;">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-[#d5d4d5]">
                    <tr>
                        ...
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Tipo</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Projeto</th>
                        <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Tarefas</th>
        ...
        <div id="tabelaProjetosOutrosColaboradores" class="mb-8">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-[#d5d4d5]">
                    ...
                    <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Tarefas</th>
                    <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Estado</th>
                    <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Colaboradores</th>
                    <th scope="col" class="px-3 py-1 text-left font-bold text-black uppercase tracking-wider border-b" style="font-size: 14px;">Ações</th>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
