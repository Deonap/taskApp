<x-app-layout>
    ...
    .btn-selecionar-colaborador, .btn-adicionar-colaborador {
        margin-left: 5px;
        cursor: pointer;
        background: none;
        border: none;
    ...
    <div class="mb-4">
        <button id="btnProjetosAbertos" class="bg-blue-500 text-white py-2 px-4 rounded mr-4">Projetos Abertos/Pendentes</button>
        <button id="btnProjetosConcluidos" class="bg-green-500 text-white py-2 px-4 rounded">Projetos Concluídos</button>
    </div>
            ...
            <table class="w-full whitespace-nowrap">
                <thead>
                    <tr class="bg-gray-100">
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo de Cliente</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nome do Projeto</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Notas Iniciais</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tempo Previsto</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Colaboradores</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tempo Gasto</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $projeto->tipoCliente->nome }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $projeto->nome }}</td>
                 {{--  <td class="px-6 py-4 whitespace-nowrap">@foreach($projeto->tarefas as $tarefa)
                            {{ $tarefa->descricao }}
                            @if(!$loop->last), @endif
                        @endforeach</td> --}}   
                        
                        <td class="px-6 py-4 whitespace-nowrap">{{ $projeto->tempo_previsto }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @foreach($projeto->users as $user)
                                <div class="flex items-center justify-between mb-2">
                                    {{ $user->name }}
                                    <button class="btn-escolher-colaborador" data-user-id="{{ $user->id }}" data-projeto-id="{{ $projeto->id }}">⬇️</button>
                                </div>
                            ...
                            <button class="btn-adicionar-colaborador" data-projeto-id="{{ $projeto->id }}">+</button>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @foreach($projeto->users as $user)
                                {{ $user->name }} - Tempo Gasto: {{ $user->tempoGasto }} horas
            ...
            <table class="w-full whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $projeto->tempo_previsto }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">@foreach($projeto->users as $user)
                            {{ $user->name }}
                            @if(!$loop->last), @endif
                        @endforeach</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $projeto->tempo_gasto }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('projetos.edit', $projeto->id) }}" class="text-blue-500 hover:text-blue-700 mr-2">Editar</a>
                            <form action="{{ route('projetos.destroy', $projeto->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
    ...
    <a href="{{ route('projetos.create', ['cliente_id' => $cliente->id]) }}" class="bg-blue-500 text-white py-2 px-4 rounded mr-4">Adicionar Projeto</a>
    </div>
