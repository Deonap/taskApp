                ...
                <table class="table-auto w-full">
                    ...
                    <tbody>
                        @foreach ($estados as $estado)
                            <tr class="border-b">
                                <td>
                                    <form action="{{ route('estado-projetos.update', $estado->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <input name="nome" value='{{ $estado->nome }}' onchange="this.form.submit()" class="border-none bg-transparent rounded-md p-2" autocomplete="off" type="text">
                                    </form>
                                </td>
                                <td>
                                    <form action="{{ route('estado-projetos.update', $estado->id) }}"
                                        class="ml-2 size-6" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <input class="rounded-sm w-full border-0" id="cor" type="color" onchange="this.form.submit()" name="cor" value="{{ $estado->cor }}">
                                    </form>
                                </td>
                                <td>
                                    <div class="flex justify-center space-x-3">
                                        <a href="{{ route('estado-projetos.edit', $estado->id) }}" title="Editar">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor"
                                                class="size-6 text-darkBlue hover:text-blue-700">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                            </svg>
                                        </a>
                                        @if (auth()->user() && auth()->user()->tipo == 'admin')
                                            <form action="{{ route('estado-projetos.destroy', $estado->id) }}" method="POST" onsubmit="return confirm('Tem a certeza?')" title="Remover">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit">
                            ...
                            <form method="POST" action="{{ route('estado-projetos.store') }}" autocomplete="off">
                                ...
                                <td>
                                    <div class="ml-2 size-6">
                                        <input class="w-full" id="cor" type="color" name="cor">
                                    </div>
                                </td>
...
<script>
    document.getElementById('btnAdicionarLinha').addEventListener('click', () => {
        document.getElementById('newStateRow').classList.remove("hidden");
    })
</script>
